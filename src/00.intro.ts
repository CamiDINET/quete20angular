...
export const firstLetterObservable = of('Wild', 'Code', 'School').pipe(map((x)=> x[0]));

// Use filter operator to allow only positive numbers
export const positiveNumbersObservable = of(-23, 543, 7, 6, 3, -234, 43).pipe(filter((number)=> number>0))
